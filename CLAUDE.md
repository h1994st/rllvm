...
**The section names are rllvm's own** (`constants.rs`): `__RLLVM,__rllvm_bc` on Mach-O, `.rllvm_bc` on ELF, COFF and WASM. Do not rename them back to LLVM's generic ones. `wasm-ld` skips `.llvmbc` and `.llvmcmd` by name — they belong to `clang -fembed-bitcode` — while concatenating every other custom section, so using those names makes WASM linking silently drop the paths. Extraction still reads the pre-rename names so older objects stay usable.
